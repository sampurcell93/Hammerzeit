(function() {
  define(['globals', 'utilities', 'jquery', 'underscore', 'easel'], function(globals, ut) {
    var $canvas, addMarker, addState, board, canvas, clear, flashStateChange, initialize, introSlider, removeState, scenecount, scenelen, setPresetBackground, setState, stage, startSlideshow, state, stateChangeEvents, states, textshadow, ticker, toggleGrid, walkingMan, _gridded, _mapheight, _mapwidth, _zoom,
      _this = this;
    ...
    board = {
      ...
      setMapSize: function(width, height) {
        _mapwidth = width;
        _mapheight = height;
        return console.log("Setting map dimensoions:", width, height);
      }
    };
    board.initialize();
