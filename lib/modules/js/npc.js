  ...
  define(["globals", "utilities", "board", "mapper", "underscore", "backbone"], function(globals, ut, board, mapper) {
    ...
    NPC = (function(_super) {
      ...
      NPC.prototype.checkElevation = function(target) {
        if (Math.abs(this.currentspace.elv - target.elv) >= 3) {
          return false;
        } else {
          return true;
        }
      ...
      NPC.prototype.checkEnterable = function(target, dx, dy) {
        try {
          if (this.checkElevation(target) === false) {
      ...
      NPC.prototype.move = function(dx, dy) {
        ...
        sheet = marker.spriteSheet = this.sheets[ut.floorToOne(dx) + "," + ut.floorToOne(dy)];
        if (!this.checkEnterable(target, dx, dy)) {
          return false;
        }
        dx *= 50;
        dy *= 50;
        if (!this.canMoveOffChunk(marker.x + dx, marker.y + dy)) {
          return false;
        }
        marker.x += dx;
        marker.y += dy;
