(function() {
  define(["globals", "utilities", "mapcreator", "underscore", "backbone", "easel", "jquery"], function(globals, ut, mapcreator) {
    var clearChunk, createBitEventRegister, getFromShorthand, loadChunk, renderChunk, setDefaultTileAttrs, tileheight, tiles, tileurl, tilewidth, _activechunk, _activeprecursor;
    ...
    renderChunk = function(bitmap, stage, vertindex) {
      ...
      _.each(bitmap, function(tile, i) {
        if ($.isArray(tile)) {
          ...
          return mapcreator.bindCreators(tile);
        }
      });
    ...
    return {
      loadChunk: function(chunk) {
        if (chunk.length) {
          ...
          chunk = mapcreator.getDefaultChunk();
        }
        return _activeprecursor = loadChunk(chunk);
      },
